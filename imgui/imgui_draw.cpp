...
void    ImFontAtlas::ClearTexData()
{
	...
	if (TexPixelsImColor32)
		ImGui::MemFree(TexPixelsImColor32);
	TexPixelsAlpha8 = NULL;
	TexPixelsImColor32 = NULL;
}
...
void    ImFontAtlas::GetTexDataAsImColor32(unsigned char** out_pixels, int* out_width, int* out_height, int* out_bytes_per_pixel)
{
	// Convert to ImColor32 format on demand
	...
	if (!TexPixelsImColor32)
	{
		...
		if (pixels)
		{
			TexPixelsImColor32 = (unsigned int*)ImGui::MemAlloc((size_t)(TexWidth * TexHeight * 4));
			const unsigned char* src = pixels;
			unsigned int* dst = TexPixelsImColor32;
			for (int n = TexWidth * TexHeight; n > 0; n--)
				*dst++ = IM_COL32(255, 255, 255, (unsigned int)(*src++));
		}
	}

	*out_pixels = (unsigned char*)TexPixelsImColor32;
	if (out_width) *out_width = TexWidth;
	if (out_height) *out_height = TexHeight;
